...
// CORREÇÃO: Função setupCourseListeners otimizada para evitar re-renderização desnecessária
function setupCourseListeners() {
    const coursesContainer = document.getElementById('courses-container');
    if (!coursesContainer) return;

    // Lógica para salvar campos de texto
    coursesContainer.querySelectorAll('.course-item').forEach(item => {
        ...
        const textInputs = [
            { selector: '.course-nome', field: 'nome', shouldRerender: true }, // Re-renderiza para atualizar o título
            { selector: '.course-regioes', field: 'regioes', shouldRerender: false }, 
            { selector: '.course-duracao', field: 'duracao', shouldRerender: false }, 
            { selector: '.course-alunos', field: 'mediaAlunos', shouldRerender: false }, 
            { selector: '.course-divulgacao', field: 'linkDivulgacao', shouldRerender: false }, 
            { selector: '.course-conteudo', field: 'linkConteudo', shouldRerender: false }, 
            { selector: '.course-tipo-outro', field: 'tipoOutro', shouldRerender: false } 
        ];

        textInputs.forEach(config => {
            const input = item.querySelector(config.selector);
            if (input) {
                // VARIÁVEIS PARA GERENCIAR O FOCO/CURSOR
                let start = input.selectionStart;
                let end = input.selectionEnd;

                input.addEventListener('focus', () => {
                    // Atualiza a posição do cursor ao receber o foco
                    start = input.selectionStart;
                    end = input.selectionEnd;
                });
                
                input.addEventListener('input', (e) => {
                    // Salva a posição atual do cursor ANTES da alteração do valor
                    start = e.target.selectionStart;
                    end = e.target.selectionEnd;

                    course[config.field] = e.target.value;
                    
                    // A re-renderização completa SÓ acontece se for o campo 'nome'
                    // Isso resolve o bug da perda de foco para os outros campos de texto.
                    if (config.shouldRerender) {
                        // Re-renderiza para atualizar o cabeçalho do curso
                        renderQuestion(currentStep);
                        
                        // RESTAURA O FOCO E A POSIÇÃO DO CURSOR
                        setTimeout(() => {
                            const updatedInput = document.querySelector(`.course-item[data-index="${index}"] ${config.selector}`);
                            if (updatedInput) {
                                updatedInput.focus();
                                updatedInput.setSelectionRange(start, end);
                            }
                        }, 0); // O setTimeout(..., 0) garante que a restauração ocorra após a pintura do novo DOM
                    }
                });
            }
        });

        // Tipos de Curso (Checkbox) - Re-renderização é NECESSÁRIA para mostrar/esconder o campo "Outro"
        item.querySelectorAll('.course-tipo').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                const value = e.target.value;

                if (e.target.checked) {
                    if (!course.tipos.includes(value)) course.tipos.push(value);
                } else {
                    course.tipos = course.tipos.filter(t => t !== value);
                }

                // Re-renderizar para atualizar o estado visual e o input 'Outro'
                renderQuestion(currentStep);
            });
        });

        // Frequência (Radio) - Re-renderização é NECESSÁRIA para atualizar o estado visual
        item.querySelectorAll('.course-frequencia').forEach(radio => {
            radio.addEventListener('change', (e) => {
                course.frequencia = e.target.value;
                // Re-renderizar para atualizar o estado visual
                renderQuestion(currentStep);
            });
        });
    });
...
// Setup Potenza Multiselect (REPLICANDO A LÓGICA DA Q8, INCLUINDO O SEARCH)
function setupPotenzaMultiselect() {
    ...
    if (trigger && dropdown) {
        ...
        document.addEventListener('click', (e) => {
            // Se o ID do container Potenza estiver presente
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Adiciona um curso vazio se não houver nenhum e o interesse em curso for true
...
function renderCheckboxWithMultiselect(question) {
    ...
    question.options.forEach(opt => {
        ...
        // Verifica se o valor é booleano (true/false) para o input
        const inputValue = opt.value === true || opt.value === false ? opt.value : '';
        
        html += `
            <div class="radio-option ${selected}" data-value="${inputValue}">
                <input type="radio" name="${question.field}" id="interesse-${inputValue}" value="${inputValue}" ${checked}>
                <label for="interesse-${inputValue}">${opt.label}</label>
            </div>
        `;
        if (opt.hasMultiselect && opt.value) {
            const show = checked ? 'show' : '';
            // Usando a estrutura do multiselect normal (Q8) com IDs específicos
...
// Corrigindo para usar a estrutura Multiselect da Q8
function renderPotenzaMultiselect() {
    ...
    let html = `
        <p style="margin: 16px 0 12px; font-weight: 600;">Quais produtos Potenza gostaria de conhecer?</p>
...
function renderCourseItem(course, index) {
    ...
    // Permite remover cursos se houver 1 ou mais
    ...
    const title = course.nome && course.nome.trim() !== '' ? 
                    ...
                    `Curso ${index + 1} (${isClosed ? 'Fechado' : 'Aberto'})`;

    // Adicionar classe de erro se a validação falhar (após a primeira tentativa de validação)
    const errorClass = course.error ? 'error-course' : '';

    return `
        <div class="course-item ${closedClass} ${errorClass}" data-index="${index}">
...
window.addCourse = function() {
    if (formData.cursos.length < 5) {
        // Fecha todos os cursos abertos antes de adicionar um novo
...
window.toggleCourse = function(index, forceClose = false) {
    ...
    if (forceClose) {
        // Verifica se os campos obrigatórios estão preenchidos antes de fechar
        if (course.nome && course.nome.trim() !== '' && course.regioes && course.regioes.trim() !== '') {
            course.isClosed = true;
            course.error = false;
        } else {
            // CORREÇÃO 2.2: Removido alert() e forçando a abertura para edição
            console.error('Atenção: Por favor, preencha o Nome e Regiões do curso antes de fechar.');
            course.isClosed = false; // Permanece aberto para edição
            course.error = true;
        }
    } else {
    ...
    if (!course.isClosed) {
        // Se está abrindo, fecha os outros
...
function renderCourseTypes(course, index) {
    ...
    types.forEach(type => {
        ...
        if (type === 'Outro') {
            const show = course.tipos.includes('outro') ? 'show' : '';
...
function setupNavigation() {
    ...
    prevBtn.addEventListener('click', () => {
        if (currentStep > 0) {
            currentStep--;
            // Limpa o estado de erro dos cursos ao voltar
            formData.cursos.forEach(c => c.error = false);
...
// Validate courses (Mantido e ajustado para classe de erro)
function validateCourses() {
    let allCoursesValid = true;
    
    // Remove a flag de erro de todos os cursos antes de re-validar
    formData.cursos.forEach(course => course.error = false);
    ...
    formData.cursos.forEach((course, index) => {
        ...
        if (!course.nome || course.nome.trim() === '' || 
            course.tipos.length === 0 || 
            !course.regioes || course.regioes.trim() === '' || 
            !course.frequencia || course.frequencia.trim() === '' || 
            !course.duracao || course.duracao.trim() === '' || 
            !course.mediaAlunos || course.mediaAlunos.trim() === '') {
            isCourseValid = false;
        }
        ...
        if (!isCourseValid) {
            ...
            course.error = true; // Marca o curso com erro
            course.isClosed = false; // Força a abertura para edição
        } else {
            course.error = false;
            course.isClosed = true; // Fecha se estiver preenchido e válido
        }
    });
    
    // Se houver cursos inválidos, re-renderiza para mostrar o feedback visual
    if (!allCoursesValid) {
        renderQuestion(currentStep);
    }
